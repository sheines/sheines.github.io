...
class Raum3D(ThreeDScene):
    def construct(self):
        ...
        # Farbe + Transparenz
        plane_surface.set_fill(BLUE_E, opacity=0.5)
        plane_surface.set_stroke(BLUE_E, width=1)
        ...
        punkt = Dot3D(point=punkt_pos, radius=0.12, color=RED)
        punkt_label = MathTex("P").scale(0.6).next_to(punkt, UP)

        self.add(punkt, punkt_label)
