...
\begin{tikzpicture}[pfeil,>=stealth,scale=0.6]
	...
	% Kurve
	\draw[ultra thick, petrol, samples=100, smooth, domain=-8:7] plot (\x,{pg(\x)});

	% Punkte
	\foreach \x in {-8,-2,2,6} %N
					\draw[fill,red] (\x,{pg(\x)}) circle(0.2);
					
					\foreach \x in {0.076602} %H
					\draw[fill,cyan] (\x,{pg(\x)}) circle(0.2);
					
					\foreach \x in {-5.8707,4.4473} %T
					\draw[fill,green] (\x,{pg(\x)}) circle(0.2);
					
					\foreach \x in {-3.4861,2.4861} %W
					\draw[fill,orange] (\x,{pg(\x)}) circle(0.2);
