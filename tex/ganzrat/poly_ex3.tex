...
\begin{tikzpicture}[pfeil,>=stealth,scale=0.6]
	...
	\draw[thick, -stealth] (0,-5) -- (0,5.5) node[left] {$y$};
	

	% Kurve
	\draw[ultra thick, petrol, samples=300, smooth, domain=-1.:6] plot (\x,{pc(\x)});
	% \draw[ultra thick, petrol, samples=150, smooth, domain=4:6] plot (\x,{pc(\x)});

	% Punkte
	\foreach \x in {-1,...,6}
		\draw[fill, darkgoldenrod] (\x,{pc(\x)}) circle (0.1);
