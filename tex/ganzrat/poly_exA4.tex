...
\begin{tikzpicture}[pfeil,>=stealth,scale=0.6]
	...
	% Kurve
	\draw[ultra thick, petrol, samples=150, smooth, domain=-9:8] plot (\x,{pj(\x)});

	% Punkte
	\foreach \x in {-9,-5,-3,0,5,8} %N
					\draw[fill,red] (\x,{pj(\x)}) circle(0.2);
					
					\foreach \x in {-4.11385,3.01839} %H
					\draw[fill,cyan] (\x,{pj(\x)}) circle(0.2);
					
					\foreach \x in {-7.80198,-1.34466,6.90877} %T
					\draw[fill,green] (\x,{pj(\x)}) circle(0.2);
					
					\foreach \x in {-6.5296,-2.8008,1.1379,5.5259} %W
					\draw[fill,orange] (\x,{pj(\x)}) circle(0.2);
