...
\begin{tikzpicture}[pfeil,>=stealth,scale=0.6]
	...
	% Kurve
	\draw[ultra thick, petrol, samples=100, smooth, domain=-8:4] plot (\x,{pf(\x)});

	% Punkte
	\foreach \x in {-7,-1,3} %N
		\draw[fill,red] (\x,{pf(\x)}) circle(0.2);
		
	\foreach \x in {-4.5726} %H
		\draw[fill,cyan] (\x,{pf(\x)}) circle(0.2);
		
	\foreach \x in {1.2393} %T
		\draw[fill,green] (\x,{pf(\x)}) circle(0.2);
		
	\foreach \x in {-1.3333} %W
		\draw[fill,orange] (\x,{pf(\x)}) circle(0.2);
